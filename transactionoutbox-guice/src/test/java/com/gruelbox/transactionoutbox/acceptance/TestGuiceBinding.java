...
class TestGuiceBinding
{
   ...
   static final class DemoModule extends AbstractModule
   {
      ...
      {
         return TransactionOutbox.builder().instantiator(GuiceInstantiator.builder().injector(injector).build()).persistor(StubPersistor.builder().build())
            .submitter(Submitter.withExecutor(Runnable::run)).transactionManager(transactionManager).listener(new TransactionOutboxListener()
            {
               @Override
